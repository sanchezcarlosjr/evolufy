...
# and `pip install evolufy` using an artifact store. Faster and more robust.
...
SHELL ["mamba", "run", "-n", "evolufy", "/bin/bash", "-c"]

# Build package
RUN tox -e build
RUN mamba env export -n evolufy -f environment.docker.yml
...
SHELL ["conda", "run", "-n", "evolufy", "/bin/bash", "-c"]
...
ENTRYPOINT ["conda", "run", "--no-capture-output", "-n", "evolufy", "evolufy web"]
